    ...
    public class DesempenoDaoImp implements DesempenoDao{
        ...
        public List<Partido> getPartidos(int id) {
            String query = "FROM Partido WHERE estado = :estado or estado = :estado2";
            boolean comp = true;
            List<Partido> lista = new ArrayList<>();
            List<Partido> partidos = entityManager.createQuery(query)
                    .setParameter("estado", "pendiente")
                    .setParameter("estado2", "inhabilitado")
                    .getResultList();

            for (Partido p : partidos) {
                if (id != 1 && p.getEstado().equals("inhabilitado")) {
                    comp = false;
                } else {
                    comp = true;
        ...
        public List<Partido> resultados() {
            String query = "FROM Partido ORDER BY id DESC";
            return entityManager.createQuery(query).getResultList();
        }
